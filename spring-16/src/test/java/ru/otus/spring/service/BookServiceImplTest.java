...
class BookServiceImplTest {
    ...
    private final Genre EXPECTING_GENRE = new Genre(3, EXPECTING_GENRE_NAME);
    private final Author EXPECTING_AUTHOR = new Author(4, EXPECTING_AUTHOR_FIO);

    @Test
    @DisplayName("удалять книгу по переданному id")
    void shouldDeleteBook() {
        bookService.delete(EXISTING_BOOK_ID);
        Mockito.verify(bookRepository, Mockito.times(1)).deleteById(EXISTING_BOOK_ID);
        Mockito.verify(bookCommentRepository, Mockito.times(1)).deleteByBookId(EXISTING_BOOK_ID);
    }
    ...
    void shouldGetBook() {
        ...
        given(bookRepository.findById(EXISTING_BOOK_ID)).willReturn(Optional.of(book));
